...
!developmentChains.includes(network.name)
    ...
    : describe("lottery unit test", function () {
          ...
          describe("enterLottery", function () {
              ...
              it("doesnt allow entrance when lottery is calculating state", async function () {
                  await lottery.enterLottery({ value: entranceFee })
          ...
          describe("checkUpkeep", function () {
              ...
              it("returns false if lottery isnt open", async function () {
                  ...
                  await network.provider.send("evm_increaseTime", [interval.toNumber() + 1])
                  await network.provider.send("evm_mine", [])
                  await lottery.performUpkeep("0x") // makes the state CALCULATING
                  const lotteryState = await lottery.getLotteryState()
                  const { upkeepNeeded } = await lottery.callStatic.checkUpkeep([])
          ...
          describe("fulfillRandomWords", function () {
              ...
              it("picks winner, resets lottery, sends money", async function () {
                  ...
                  for (let i = startingAccountIndex; i < startingAccountIndex + entrants; i++) {
                      const accountConnected = lottery.connect(accounts[i])
                      await accountConnected.enterLottery({ value: entranceFee })
                  }
                  const startingTimeStamp = await lottery.getLatestTimeStamp()

                  // performUpkeep mocking keepers chainlink
                  // mocks fulfillrandomworks
                  await new Promise(async (resolve, reject) => {
                      lottery.once("WinnerPicked", async () => {
                          console.log("found winner pickings!")
                          try {
